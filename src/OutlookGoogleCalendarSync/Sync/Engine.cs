...
namespace OutlookGoogleCalendarSync.Sync {
    public class Engine {
        ...
        public void RegisterForPushSync() {
            if (!Settings.Instance.ActiveCalendarProfile.OutlookPush || Settings.Instance.ActiveCalendarProfile.SyncDirection.Id == Sync.Direction.GoogleToOutlook.Id) return;
        ...
        public void Start(Boolean updateSyncSchedule = true) {
            ...
            try {
                ...
                mainFrm.Console.BuildOutput("Syncing from " + Settings.Instance.ActiveCalendarProfile.SyncStart.ToShortDateString() +
                ...
                try {
                    ...
                    if (ex.InnerException != null && ex.InnerException is Google.GoogleApiException &&
                        ...
                        ex.Data.Add("OGCS", "ERROR: Unable to connect to the Google calendar" +
                            (Settings.Instance.ActiveCalendarProfile.SyncInterval == 0 ? ". Please try again after " + tryAfter : ", but OGCS is all set to automatically try again after "+ tryAfter));
