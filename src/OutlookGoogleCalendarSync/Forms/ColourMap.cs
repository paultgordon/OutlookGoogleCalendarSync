...
namespace OutlookGoogleCalendarSync.Forms {
    public partial class ColourMap : Form {
        ...
        private void loadConfig() {
            try {
                if (Settings.Instance.ActiveCalendarProfile.ColourMaps.Count > 0) colourGridView.Rows.Clear();
                foreach (KeyValuePair<String, String> colourMap in Settings.Instance.ActiveCalendarProfile.ColourMaps) {
        ...
        private void btOK_Click(object sender, EventArgs e) {
            ...
            try {
                ...
                if (!string.IsNullOrEmpty(oDuplicates) && (Settings.Instance.ActiveCalendarProfile.SyncDirection.Id == Sync.Direction.OutlookToGoogle.Id || Settings.Instance.ActiveCalendarProfile.SyncDirection.Id == Sync.Direction.Bidirectional.Id)) {
                    ...
                    return;
                } else if (!string.IsNullOrEmpty(gDuplicates) && (Settings.Instance.ActiveCalendarProfile.SyncDirection.Id == Sync.Direction.GoogleToOutlook.Id || Settings.Instance.ActiveCalendarProfile.SyncDirection.Id == Sync.Direction.Bidirectional.Id)) {
            ...
            try {
                ...
                Settings.Instance.ActiveCalendarProfile.ColourMaps.Clear();
                foreach (DataGridViewRow row in colourGridView.Rows) {
                    if (row.Cells["OutlookColour"].Value == null || row.Cells["OutlookColour"].Value.ToString().Trim() == "") continue;
                    try {
                        Settings.Instance.ActiveCalendarProfile.ColourMaps.Add(row.Cells["OutlookColour"].Value.ToString(), GoogleOgcs.EventColour.Palette.GetColourId(row.Cells["GoogleColour"].Value.ToString()));
