    ...
    public class EventColour {
        ...
        public List<Palette> ActivePalette {
            get {
                ...
                if (Settings.Instance.ActiveCalendarProfile.UseGoogleCalendar == null) return activePalette;

                if (Settings.Instance.ActiveCalendarProfile.UseGoogleCalendar.ColourId == null) {
                    List<GoogleCalendarListEntry> gCals = GoogleOgcs.Calendar.Instance.GetCalendars();
                    Settings.Instance.ActiveCalendarProfile.UseGoogleCalendar.ColourId = gCals.Find(c => c.Id == Settings.Instance.ActiveCalendarProfile.UseGoogleCalendar.Id).ColourId;
                }
                ...
                foreach (Palette cal in calendarPalette) {
                    if (cal.Id == Settings.Instance.ActiveCalendarProfile.UseGoogleCalendar.ColourId) {
                        currentCal = cal;
                        break;
                    }
                }
