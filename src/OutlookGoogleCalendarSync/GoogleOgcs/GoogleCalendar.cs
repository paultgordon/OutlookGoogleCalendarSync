...
namespace OutlookGoogleCalendarSync.GoogleOgcs {
    ...
    public class Calendar {
        ...
        public List<Event> GetCalendarEntriesInRange(DateTime from, DateTime to) {
            ...
            if (Settings.Instance.ActiveCalendarProfile.ExcludeDeclinedInvites) {
            ...
            if ((IsDefaultCalendar() ?? true) && Settings.Instance.ActiveCalendarProfile.ExcludeGoals) {
        ...
        private Event createCalendarEntry(AppointmentItem ai) {
            ...
            if (Settings.Instance.ActiveCalendarProfile.AddAttendees && ai.Recipients.Count > 1 && !APIlimitReached_attendee) { //Don't add attendees if there's only 1 (me)
                if (ai.Recipients.Count > Settings.Instance.ActiveCalendarProfile.MaxAttendees) {
            ...
            if (Settings.Instance.ActiveCalendarProfile.AddAttendees && ai.Recipients.Count > 1 && !APIlimitReached_attendee) {
                ...
                if (ai.Recipients.Count >= 200) {
                    Forms.Main.Instance.Console.Update(OutlookOgcs.Calendar.GetEventSummary(ai) + "<br/>Attendees will not be synced for this meeting as it has " +
                        ...
                        ev.Attendees != null && ev.Attendees.Count > Settings.Instance.ActiveCalendarProfile.MaxAttendees && ai.Recipients.Count <= Settings.Instance.ActiveCalendarProfile.MaxAttendees) {
        ...
        public void ReclaimOrphanCalendarEntries(ref List<Event> gEvents, ref List<AppointmentItem> oAppointments, Boolean neverDelete = false) {
            if (Settings.Instance.ActiveCalendarProfile.SyncDirection == Sync.Direction.GoogleToOutlook) return;
                ...
                for (int o = ai.Recipients.Count; o > 0; o--) {
                    ...
                    foreach (Google.Apis.Calendar.v3.Data.EventAttendee attendee in ev.Attendees ?? Enumerable.Empty<Google.Apis.Calendar.v3.Data.EventAttendee>()) {
                        ...
                        if (ogcsAttendee.Email != null && (recipientSMTP.ToLower() == ogcsAttendee.Email.ToLower())) {
                            ...
                            if (Settings.Instance.ActiveCalendarProfile.CloakEmail != ogcsAttendee.IsCloaked()) {
        ...
        private String getAvailability(OlBusyStatus oBusyStatus, String gTransparency) {
            ...
            try {
                Enum.TryParse(Settings.Instance.ActiveCalendarProfile.AvailabilityStatus, out fbStatus);
        ...
        public static Boolean? IsDefaultCalendar() {
            if (!Settings.InstanceInitialiased() || (Settings.Instance.ActiveCalendarProfile.UseGoogleCalendar == null || string.IsNullOrEmpty(Settings.Instance.GaccountEmail)))
                return null;

            return Settings.Instance.ActiveCalendarProfile.UseGoogleCalendar.Id == Settings.Instance.GaccountEmail;
        }
        #endregion
